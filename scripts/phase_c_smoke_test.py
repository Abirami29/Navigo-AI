...
def run() -> None:
    ...
    print(
        f"Eligible restaurants: {len(eligible)} — wheelchair='no' is still excluded (a real "
        "hard filter, since Grandpa needs a wheelchair), but missing dietary data no longer "
        "hides a restaurant outright; it gets annotated with confirmed/unconfirmed instead."
    )
    for r in eligible[:5]:
        print(
            f"  {r['name']} | wheelchair={r['osm_wheelchair']} "
            f"| confirmed={r.get('dietary_confirmed')} | unconfirmed={r.get('dietary_unconfirmed')}"
        )
